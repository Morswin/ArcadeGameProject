...
void Map::DisplayFloorAndWall(Player& player, Renderer& renderer)
{
    ...
    const int lowX = static_cast<int>(std::floor(playerPosition.x - m_VisibleRange.x));
    const int lowY = static_cast<int>(std::floor(playerPosition.y - m_VisibleRange.y));
    const int highX = static_cast<int>(std::ceil(playerPosition.x + m_VisibleRange.x));
    const int highY = static_cast<int>(std::ceil(playerPosition.y + m_VisibleRange.y));
...
std::vector<glm::vec2> Map::PopulateMapWithEnemies(double percentage)
{
    ...
    const size_t _enemy_count = std::min(static_cast<size_t>(std::ceil(_possiblePositions.size() * percentage)), _possiblePositions.size());
    std::vector<glm::vec2> _enemy_positions;
    _enemy_positions.reserve(_enemy_count);
    for (size_t i = 0; i < _enemy_count; i++)
    {
        _enemy_positions.emplace_back(
            _possiblePositions[i].x,
            _possiblePositions[i].y
            );
    }
    return _enemy_positions;
}
